...
y_train = one_hot_encode(y_train, 11)

eval_data = np.load('./evaluation_28.npz')
x_eval = eval_data['x']
y_eval = eval_data['y']
y_eval = one_hot_encode(y_eval, 11)
...
def cnn_model(alpha = 0.):
    ...
    model.add(Conv2D(32, (3, 3), input_shape=(26, 26 ,32),
                            activation='relu'
                            ,name='conv2',kernel_regularizer=regularizers.l2(alpha))) 
    # input 32,26,26,32 output 32,24,24,32
    model.add(Conv2D(32, (3, 3), input_shape=(24, 24 ,32),
                            activation='relu'
                            ,name='conv3',kernel_regularizer=regularizers.l2(alpha))) 
    # input 32,24,24,32 output 32,22,22,32
